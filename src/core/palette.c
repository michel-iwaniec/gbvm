...
void SGBTransferPalettes(UBYTE palettes) BANKED {
    ...
    if (palettes & SGB_PALETTES_01) {
        data.cmd = (SGB_PAL_01 << 3) | 1;
        // gbdk-nes-TODO: memcpy(data.palettes, &BkgPalette[4], sizeof(palette_entry_t));
        // gbdk-nes-TODO: memcpy(&data.palettes[4], &BkgPalette[5].c1, sizeof(palette_entry_t) - sizeof(UWORD));
        sgb_transfer((void *)&data);
    }
    if (palettes & SGB_PALETTES_23) {
        data.cmd = (SGB_PAL_23 << 3) | 1;
        // gbdk-nes-TODO: memcpy(data.palettes, &BkgPalette[6], sizeof(palette_entry_t));
        // gbdk-nes-TODO: memcpy(&data.palettes[4], &BkgPalette[7].c1, sizeof(palette_entry_t) - sizeof(UWORD));
        sgb_transfer((void *)&data);
    }
