...
UBYTE scroll_viewport(parallax_row_t * port) {
    if (port->next_y) {
        ...
        if (current_col == new_col - 1) {
            ...
            UBYTE x = shift_col - SCREEN_PAD_LEFT + SCREEN_TILE_REFRES_W; //HACK: Push updates into blanked column
            scroll_load_col(x, port->start_tile, port->tile_height);
        } else if (current_col == new_col + 1) {
            // Render left column
            //UBYTE x = MAX(0, shift_col - SCREEN_PAD_LEFT);
            UBYTE x = MAX(0, shift_col - SCREEN_PAD_LEFT) + 1; //HACK: Push updates into blanked column
            scroll_load_col(x, port->start_tile, port->tile_height);
        ...
        if (!needs_full_update && current_col == new_col - 1) {
            ...
            x = new_col - SCREEN_PAD_LEFT + SCREEN_TILE_REFRES_W; //HACK: Push updates into blanked column
            ...
            x = MAX(0, new_col - SCREEN_PAD_LEFT) + 1; //HACK: Push updates into blanked column
            UBYTE y = MAX(0, MAX((new_row - SCREEN_PAD_TOP), port->start_tile));
            full_y = MAX(0, (new_row - SCREEN_PAD_TOP));
            scroll_queue_col(x, y);
...
void scroll_render_rows(INT16 scroll_x, INT16 scroll_y, BYTE row_offset, BYTE n_rows) {
    ...
    //UBYTE x = MAX(0, (scroll_x >> 3) - SCREEN_PAD_LEFT) + 1; //HACK: Push updates into blanked column
...
void scroll_queue_row(UBYTE x, UBYTE y) {
    ...
    pending_w_i = SCREEN_TILE_REFRES_W + 1; // HACK: Refresh an additional column, like in large_map example
}
...
void scroll_load_row(UBYTE x, UBYTE y) NONBANKED {
    ...
    if (_is_CGB) {  // Color Column Load
        ...
        if(image_tile_width)
            ...
            set_bkg_submap_attributes_(x, y, SCREEN_TILE_REFRES_W+1, 1, image_attr_nes_ptr, image_tile_width);
    ...
    if(image_tile_width)
        ...
        set_bkg_submap(x, y, MIN(SCREEN_TILE_REFRES_W+1, image_tile_width), 1, image_ptr, image_tile_width);
