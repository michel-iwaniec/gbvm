...
void platform_init(void) BANKED {
    ...
    camera_deadzone_x_subpx = PX_TO_SUBPX(PLATFORM_CAMERA_DEADZONE_X);
    camera_deadzone_y_subpx = PX_TO_SUBPX(PLATFORM_CAMERA_DEADZONE_Y);
...
void platform_update(void) BANKED {
    ...
    if (on_ladder) {
        ...
        if (pl_vel_y > 0) {
            ...
            UWORD x_mid_coord = PLAYER.pos.x + PLAYER.bounds.left + p_half_width + 1;
            while (tile_y <= new_tile_y) {
                UBYTE col = tile_at(SUBPX_TO_TILE(x_mid_coord), tile_y);
                UWORD tile_x_coord = SUBPX_SNAP_TILE(x_mid_coord);
                ...
                if (IS_ON_SLOPE(col)) {
                    if ((col & COLLISION_SLOPE) == COLLISION_SLOPE_45_RIGHT) {
                        ...
                        slope_y_coord = TILE_TO_SUBPX(tile_y) + (x_offset >> 1) - PLAYER.bounds.bottom + PX_TO_SUBPX(4) - 1;
            ...
            while (tile_start != tile_end) {
                if (tile_at(tile_start, tile_y) & COLLISION_BOTTOM) {
                    new_y = TILE_TO_SUBPX((UBYTE)(tile_y + 1)) - PLAYER.bounds.top + 1;
                    pl_vel_y = 0;
                    break;
                }
                tile_start++;
