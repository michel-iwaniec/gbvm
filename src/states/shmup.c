...
void shmup_update(void) BANKED {
            ...
            if (PLAYER.dir == DIR_DOWN) {
                ...
                while (tile_start != tile_end) {
                    if (tile_at(tile_start, tile_y) & COLLISION_BOTTOM) {
                        new_pos.y = TILE_TO_SUBPX((UBYTE)(tile_y + 1)) - PLAYER.bounds.top + 1;
                        break;
                    }
                    tile_start++;
                }
    ...
    if (!shooter_reached_end) {
        ...
        point_translate_dir(&PLAYER.pos, shooter_direction, shooter_scroll_speed << 1);
#else
